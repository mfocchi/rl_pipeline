...
class DDPQuadrupedRobot:
    def __init__(self, robot, lfFootName, lhFootName, rfFootName, rhFootName, baseName):
        # Robot
        self.robot = robot
        ...
        self.state_lb = np.delete(self.state.lb, 6)
        self.state_ub = np.delete(self.state.ub, 6)
        # actuation bounds (size = state.nq-6)
        self.actuation_lb = -self.rmodel.effortLimit[6:]
        self.actuation_ub = self.rmodel.effortLimit[6:]

        # Getting the frame id for all the feet and for the base
        lfFootId = self.rmodel.getFrameId(lfFootName)
        lhFootId = self.rmodel.getFrameId(lhFootName)
        rfFootId = self.rmodel.getFrameId(rfFootName)
        rhFootId = self.rmodel.getFrameId(rhFootName)
        self.feetId = {'lf': lfFootId, 'lh': lhFootId, 'rf': rfFootId, 'rh':rhFootId}
        ...
        self.mu = 0.0
        self.RSurf = {'lf': np.full([3,3], np.nan), 'lh': np.full([3,3], np.nan), 'rf': np.full([3,3], np.nan),
                            'rh':np.full([3,3], np.nan)}
    ...
    def createContactModel(self, supportFeet, feetRef, BaumgarteGains):
        ...
        for foot in supportFeet:
            ...
            supportContactModel = crocoddyl.ContactModel3D(self.state, idx, footRef.translation,pinocchio.LOCAL_WORLD_ALIGNED, self.actuation.nu,
                                                           BaumgarteGains)
    ...
    def createJumpOnHorizontalTerrainProblem(self, x0, problemDescription):
        ...
        flyingUpT = np.sqrt(2*delta_z/(-self.rmodel.gravity.linear[2]))
        flyingUpKnots = int(flyingUpT/timeStep)
        basePoseApex = basePoseInit.copy()
        basePoseApex.translation = comApex + np.array([0.,0.,0.03])
        ...
        basePoseFinal.translation = comFinal + np.array([0., 0., 0.03])
        ...
        for k in range(1, flyingDownKnots+1):
            ...
            for foot in feetPoseInit:
                feetPoseRef[foot] = basePoseInit.actInv(basePoseRef).act(feetPoseInit[foot])
                self.footRef.append(feetPoseRef[foot].translation)
            self.comRef.append(comRef)
            self.basePoseRef.append(pin.rpy.matrixToRpy(basePoseRef.rotation))
            model = self.createFlyingActionModel(timeStep=timeStep,
                                                 feetRef=feetPoseRef,
    ...
    def createContactsAndBaseActionModel(self, timeStep, supportFeet=None, feetRef=None, basePoseRef=None, weights=None):
